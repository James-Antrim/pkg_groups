    ...
    public function postflight(string $type, InstallerAdapter $adapter): bool
    {
        if ($type === 'install') {
            ...
            $query->select($db->qn('params'))->from($db->qn('#__extensions'))
                ->where($db->qn('name') . ' = ' . $db->q('mod_thm_groups_profiles'));
            $db->setQuery($query);
            ...
            if ($params = $db->loadResult()) {
                ...
                $query->update($db->qn('#__extensions'))
                    ...
                    ->where($db->qn('name') . ' = ' . $db->q('mod_groups_profiles'));
                $db->setQuery($query);
                $db->execute();
            }
            ...
            $query->update($db->qn('#__modules'))
                ->set($db->qn('module') . ' = ' . $db->q('mod_groups_profiles'))
                ->where($db->qn('module') . ' = ' . $db->q('mod_thm_groups_profiles'));
            $db->setQuery($query);
            $db->execute();
        }
